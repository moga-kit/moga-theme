<div class="mb-3">
    ...
    [{if $oxcmp_user->oxuser__oxcountry->value}][{$oxcmp_user->oxuser__oxcountry->value}]<br>[{/if}]
    [{if $oxcmp_user->oxuser__oxusername->value}][{$oxcmp_user->oxuser__oxusername->value}]<br>[{/if}]
    [{if $oxcmp_user->oxuser__oxfon->value}][{oxmultilang ident="PHONE"}] [{$oxcmp_user->oxuser__oxfon->value}][{/if}]
    [{if $oxcmp_user->oxuser__oxfon->value && $oxcmp_user->oxuser__oxfax->value}] | [{else}]<br>[{/if}]
