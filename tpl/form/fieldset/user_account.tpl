...
<div class="mb-3[{if $aErrors.oxuser__oxpassword}] oxInValid[{/if}]">
    [{block name="user_account_password"}]
        ...
        <input id="userPassword" class="form-control" type="password" name="lgn_pwd" value="[{$lgn_pwd}]" required>
    [{/block}]
</div>
<div class="mb-3[{if $aErrors.oxuser__oxpassword}] oxInValid[{/if}]">
    [{block name="user_account_confirmpwd"}]
        <label class="req">[{oxmultilang ident="CONFIRM_PASSWORD"}]</label>
        <input id="userPasswordConfirm" class="form-control" type="password" name="lgn_pwd2" value="[{$lgn_pwd2}]" required>
    [{/block}]
</div>
